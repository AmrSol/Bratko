...
timetable([ riva at 12:10, limone at 12:32, malcesine at 12:45 ]).
...
schedule( Start, Destination,[ depart(Start), arrive(Next) | Rest]) :-
    list( Rest),                % Rest of scheule is a list, try short schedules first
    sail(Start, Next),
    rest_schedule( Next, Destination, Rest).

rest_schedule( Place, Place, []). % Already at destination - empty schedule

rest_schedule( Current Place, Destination, [ arrive(Next) | Rest]) :-
    sail( CurrentPlace, Next),  % Stay onboard, continue immediatedely to town Next
    rest_schedule(Next, Destination, Rest).

rest_schedule( Place at Time1, Destinatoin, [ stay( Place, Time1, Time2) | Rest]) :-
...
% this is true if there is a direct connection between Place1 at Time1 (departing) and Place 2 at Time2 (arriving)

sail( Placetime1, PlaceTime2):-
    timetable(Route),
    conc(_, [ PlaceTime1, PlaceTime2| _], Route).
